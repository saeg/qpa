...
def get_all_pattern_summaries():
    ...
    try:
        ...
        print(f"Successfully found {len(patterns_list)} patterns available from the API.")
        return patterns_list

    except requests.exceptions.RequestException as e:
        print(f"[FATAL ERROR] Could not fetch the master list of patterns. Error: {e}")
    except KeyError:
        print("[FATAL ERROR] The JSON structure of the master list seems to have changed. Could not find '_embedded' or 'patternModels'.")
...
def download_quantum_pattern_details(pattern_summaries):
    ...
    for i, pattern_summary in enumerate(pattern_summaries, 1):
        ...
        headers['referer'] = f"https://patternatlas.planqk.de/pattern-languages/{language_id}/{pattern_id}"
        ...
        try:
            ...
            sections_to_extract = ["Intent", "Alias", "Context", "Forces", "Solution", "Result"]
            ...
            print(f"  [ERROR] Could not fetch details for {pattern_name}. URL: {rendered_content_url}. Error: {e}")
        ...
        if detailed_patterns:
            ...
            for i, pattern in enumerate(detailed_patterns, 1):
                ...
                for key, value in pattern.items():
                    if key != 'name':
                        wrapped_text = textwrap.fill(value, width=100, initial_indent='    ', subsequent_indent='    ')
            ...
            try:
                ...
                print("="*80)
                print(f"Successfully saved {len(detailed_patterns)}/{len(patterns_to_process)} patterns to 'quantum_patterns.json'")
                print("="*80)
            except IOError as e:
