...
def is_repo_relevant(repo):
    ...
    if repo.stargazers_count < MIN_STARS:
        print(
            f"[SKIPPING] {repo.full_name}: Not enough stars ({repo.stargazers_count} < {MIN_STARS})."
        )
        return False, None
    ...
    try:
        ...
        if contributors_count < MIN_CONTRIBUTORS:
            print(
                f"[SKIPPING] {repo.full_name}: Not enough contributors ({contributors_count} < {MIN_CONTRIBUTORS})."
            )
            return False, None
    except GithubException as e:
        if e.status == 403:
            print(
                f"Warning: Could not fetch contributors for {repo.full_name} due to API limitations ({e.status}). Skipping.",
                file=sys.stderr,
            )
            return False, None
