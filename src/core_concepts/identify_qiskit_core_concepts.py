        ...
        if raw_docstring and (docstring := clean_qiskit_docstring(raw_docstring)):
            ...
            if full_concept_name not in self.found_concepts:
                ...
                self.found_concepts[full_concept_name] = {
                    "name": full_concept_name, "summary": docstring.strip().split('\n\n')[0].strip().replace('\n', ' '),
                    "docstring": docstring.strip(), "source_code": ast.get_source_segment(self.source_text, node),
                    "type": "Class", "is_target_subclass": any(base in TARGET_BASE_CLASSES for base in base_names),
...
def _find_concepts_in_file(py_path: Path, sdk_root: Path) -> list:
    try:
        ...
        if len(source_text.strip()) < 50: return []
...
def extract_qiskit_concepts() -> List[Dict[str, Any]]:
    if not QISKIT_PROJECT_ROOT.is_dir():
        print(f"Error: Qiskit project root not found at '{QISKIT_PROJECT_ROOT.resolve()}'")
        return []
    ...
    if not search_path.is_dir():
        ...
        return []
    all_py_files = [
        py_file for py_file in search_path.rglob("*.py")
        if not any(part in EXCLUDE_SUBDIRS for part in py_file.relative_to(search_path).parts)
    ]
    if not all_py_files:
...
def deduplicate_by_naming_convention(concepts_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for module, concepts_in_module in concepts_by_module.items():
        classes = {c['name'].split('.')[-1]: c for c in concepts_in_module if c['type'] == 'Class'}
        functions = {f['name'].split('.')[-1]: f for f in concepts_in_module if f['type'] == 'Function'}
        if not classes or not functions: continue
...
def deduplicate_concepts_semantic(concepts_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    if not concepts_data: return []
    print("\n--- Deduplicating concepts using semantic similarity ---")
    model = SentenceTransformer(EMBEDDING_MODEL_NAME)
    print("Generating embeddings for concept summaries...")
    embeddings = model.encode([c['summary'] for c in concepts_data], convert_to_tensor=True, show_progress_bar=True)
    clusters = []
    ...
    for cluster in clusters:
        if len(cluster) == 1:
            ...
            continue
        best_concept = max(cluster, key=lambda c: (
...
def _save_source_code_snippets(concepts_data: List[Dict[str, Any]]):
    ...
    for concept in concepts_data:
        if source_code := concept.get("source_code"):
            sanitized_name = concept['name'].replace('/', '_').replace('.', '_').lstrip('_')
            try:
                (SOURCE_SNIPPETS_DIR / f"{sanitized_name}.py").write_text(source_code, encoding='utf-8')
                count += 1
            except Exception as e:
...
def save_concepts_to_json(concepts_data: List[Dict[str, Any]]):
    try:
        json_data = [{k: v for k, v in item.items() if k != 'source_code'} for item in concepts_data]
